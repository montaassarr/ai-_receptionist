...
function ChatTestPage() {
        ...
        try {
            ...
            if (data?.assistant_id) {
                ...
                setMessages([
                    {
                        role: 'assistant',
                        content: data.first_message || "Hi! Thank you for calling. How can I help you today?",
                        timestamp: new Date()
                    }
                ]);
            }
    ...
    const sendMessage = async ()=>{
        ...
        setLoading(true);
        try {
            // Simulate AI response via backend webhook
            const response = await fetch('/api/v1/vapi/chat', {
                method: 'POST',
                headers: {
    ...
    const getSimulatedResponse = (userInput)=>{
        ...
        if (lower.includes('book') || lower.includes('appointment')) {
            return "I'd be happy to help you book an appointment! What service are you interested in, and when would you like to come in?";
        }
        if (lower.includes('available') || lower.includes('time') || lower.includes('slot')) {
            return "Let me check our availability. We have openings at 9am, 10am, 11am, 2pm, and 3pm. Which time works best for you?";
        }
        if (lower.includes('price') || lower.includes('cost') || lower.includes('how much')) {
            return "Our services include: Women's Haircut ($45), Men's Haircut ($30), Full Color ($85), and Highlights ($120). Which service interests you?";
        }
        if (lower.includes('hour') || lower.includes('open')) {
            return "We're open Tuesday through Saturday, 9am to 6pm. We're closed on Sunday and Monday.";
        }
        return "I'm here to help! You can ask about our services, pricing, availability, or book an appointment.";
    };
...
//# sourceMappingURL=frontend_next_206fc70c._.js.map