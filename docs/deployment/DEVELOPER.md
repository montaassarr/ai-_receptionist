...
graph TD
    ...
    Backend -->|Voice| LiveKit[LiveKit Cloud]
...
- **`business_config`**: **Encrypted** API keys (`openai`, `groq`, `twilio`, etc.) & automation toggles. LiveKit credentials live in env vars only.
...
3. **Usage**: Keys are decrypted on-the-fly *only* when needed (e.g., calling Groq/OpenAI). LiveKit auth tokens are generated centrally from platform env vars.
